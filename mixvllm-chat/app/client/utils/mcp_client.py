...
def get_mcp_config(config_path: Optional[str] = None) -> MCPConfig:
    ...
    # Ensure config_path points to a file
    if config_path and Path(config_path).is_dir():
        config_path = Path(config_path) / "mcp_servers.yaml"

    if _mcp_config is None or (config_path is not None and str(_mcp_config.config_path) != str(config_path)):
        logger.debug(f"Initializing MCPConfig with path: {config_path}")
